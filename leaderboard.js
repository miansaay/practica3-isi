...
PlayersService = {
  ...
  takePlayer: function (playerId) {
    Players.update(playerId, {$inc: {score: -5}});
  },
  addPlayer: function (playerName) {
    Players.insert({name:playerName, score: 0, createdBy: Meteor.userId()});
  },
...
if (Meteor.isClient) {
  ...
  Template.leaderboard.events({
    ...
    'click input.decrement': function () {
      PlayersService.takePlayer(Session.get("selected_player"));
    },
  ...
  Template.addPlayerForm.events({
    'submit form': function(event){
        ...
        var NamePlayer = $("[name=playerName]").val();
        //console.log(NamePlayer);
        PlayersService.addPlayer(NamePlayer);
        $("[name=playerName]").val("");
    }
  });
